...
def main():
    ...
    # dibujos
    draw_uncurated_result_figure(os.path.join(config.result_dir, 'obj1__0.png'), load_Gs(url_objects), cx=0, cy=0, cw=128, ch=128, rows=3, lods=[0,1,2,2,3,3], seed=5)
    draw_style_mixing_figure(os.path.join(config.result_dir, 'obj2__0.png'), load_Gs(url_objects), w=128, h=128, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
    draw_noise_detail_figure(os.path.join(config.result_dir, 'obj3__0.png'), load_Gs(url_objects), w=128, h=128, num_samples=100, seeds=[1157,1012])
    draw_noise_components_figure(os.path.join(config.result_dir, 'obj4__0.png'), load_Gs(url_objects), w=128, h=128, seeds=[1967,1555], noise_ranges=[range(0, 18), range(0, 0), range(8, 18), range(0, 8)], flips=[1])
    draw_truncation_trick_figure(os.path.join(config.result_dir, 'obj5__0.png'), load_Gs(url_objects), w=128, h=128, seeds=[91,388], psis=[1, 0.7, 0.5, 0, -0.5, -1])
